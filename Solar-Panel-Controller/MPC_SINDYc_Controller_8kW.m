...
% Load your data
data = readtable('solar_data_8kW_mar.xlsx', 'VariableNamingRule', 'preserve');
...
for d_idx = 1:length(unique_days)
    ...
    for k = 1:length(day_indices)
        ...
        % Calculate the output power, ensuring it is non-negative
        P_output_day(k) = max(P_solar_k + P_battery_day(k), 0);

        % Calculate grid energy consumed, draw from the grid if needed
